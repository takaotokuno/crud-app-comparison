...
- `staff` は商品の閲覧と在庫操作、`viewer` は閲覧のみとし、権限のない登録・編集・削除の導線は表示しない。
...
データがない場合は表の代わりに空状態メッセージを表示し、登録権限があるユーザーには新規登録への導線を併記する。
...
│ 理由・メモ                    [                           ]  │
│                                                              │
│ ! 出庫後の在庫が0未満になる場合などのエラーをここに表示     │
│                                      [キャンセル] [登録]     │
└──────────────────────────────────────────────────────────────┘
```

取引種別は入庫・出庫・調整から選択する。登録時に現在在庫数を更新し、同時に在庫取引履歴を作成する。
